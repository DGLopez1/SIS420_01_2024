...
from cuatroenraya.objects import Player, Agent, Board

class GameScene(SceneBase):
    def __init__(self, agente="axr-master/axr/examen.pickle"):
        super().__init__()
        ...
        for i in range(3):  # Cambiado a 3 para un tablero 4x4
            rect = pygame.Rect(
                (i+1)*(size[0] / 4) - 0.5*rect_width, 0, rect_width, size[1])  # Cambiado a 4 para un tablero 4x4
            pygame.draw.rect(self.background, (255, 255, 255), rect)
            rect = pygame.Rect(
                0, (i+1)*(size[1] / 4) - 0.5*rect_height, size[0], rect_height)  # Cambiado a 4 para un tablero 4x4
            pygame.draw.rect(self.background, (255, 255, 255), rect)
    ...
    def process_input(self, scene_manager):
        for event in pygame.event.get():
            ...
            # click en la cuadrícula
                ...
                x = int(event.pos[0] // (self.size[0] / 4))  # Cambiado a 4 para un tablero 4x4
                y = int(event.pos[1] // (self.size[1] / 4))  # Cambiado a 4 para un tablero 4x4
                # movimiento jugador
                if (y, x) in self.board.valid_moves():
                    ...
                    if game_result == self.player.symbol:
                ...
                game_result = self.board.is_game_over()
                if game_result == self.agent.symbol:
